...
| Hook | Shim | Behavior |
|---|---|---|
| `on_session_start` | `hooks/on_session_start.sh` | Pin the chat to one `sessions` row and print `seaglass session start` (the profile, the preferences and any setup step, from the server's `start_session`) for Hermes to inject as context. |
