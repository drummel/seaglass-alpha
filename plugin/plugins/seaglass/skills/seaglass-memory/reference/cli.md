...
The `seaglass` binary drives the same Seaglass backend as the MCP tools. Reach
for it when you have a shell and the MCP tools are not loaded, when a scripted
loop is cheaper than a tool call per item, or when a body is large enough to
want `--via-upload`. Everything the main skill says about *when* to read,
*when* to write, sensitivity, the house voice, the cost-routing modes and the
fields you never pass applies unchanged here; this document only maps those
decisions onto commands. It is not a second set of rules.
...
You reached this document because the shell is the surface. The names the main
skill uses for the read and write operations -- `search`, `store_memory`,
`store_document`, `update_memory`, `reconsolidate_memory` -- are the MCP tools'
names, and on this path they are not callable. The same decisions reach the
same backend as `seaglass` command lines, so express every one of them that
way: run the command, or show the user the exact command line you would run.
...
this table only says what it looks like once it is. The rest of this document
expands each row.
...
- **`seaglass: command not found`**: the CLI is not installed. Tell the user
  once to install it with `curl -fsSL https://raw.githubusercontent.com/drummel/seaglass-alpha/main/cli/install.sh | bash`.
  Do not retry in a loop.
- **Exit code 5 from any command, or `seaglass whoami` non-zero**: installed
  but not authenticated. Offer to run `seaglass auth login` for the user
  rather than only describing it: say what it does and ask once, then run it
  on yes. It opens a browser and blocks until the user approves, so confirm
  before firing it. On approval the bearer token is cached at
  `~/.config/seaglass/token`, and every later `seaglass` call picks it up. For
  a non-default deployment they `export SEAGLASS_URL=https://...` before
  logging in. With no browser at hand (a cloud container), call the
  `cli_handoff` MCP tool instead and run the `seaglass auth redeem <code>`
  command it returns.
...
## Orientation: `seaglass me`

```bash
seaglass me --json
```

Returns the user's profile (the same markdown as `seaglass://profile`, with
its `## Mode` section) plus their canonical self page. It is not a gate in
front of a read: if the user's first message asks about a person, project or
topic, go straight to `seaglass search`. In Claude Code the `SessionStart`
hook already injects this profile, so re-run it only for a fresh fetch.
...
- `--sensitivity private|sensitive` sets the level; a `<private>` span in
  `--content` forces `private` server-side whatever you pass.
- `--event-time <ISO instant>` sets `event_time`, under the same rule as the
  argument: only from a date the conversation gives you.
...
`flag_sensitive`, `flag_private` and `redact`. The correction rule is the main
skill's: a newer fact is one `memory store --supersedes`, never
retract-then-store.
...
and excerpt: re-read and re-apply. Use `page edit` and friends in agent mode
under the same rule as the tools; in server mode capture with `memory store`
and `document store` and let the worker author.
...
`success: true`, means the fact is NOT in Seaglass. The main skill's rule
holds: one quiet retry when the error looks transient, then tell the user
plainly that the capture did not happen. Every state-changing response
carries the same `receipt` the tools return; report what it says and nothing
more.
...
| 5 | auth failure | offer `seaglass auth login` (or `cli_handoff` with no browser); do not retry |
